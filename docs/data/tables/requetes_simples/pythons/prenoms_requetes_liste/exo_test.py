...
filles_ = [entree for entree in prenoms if entree[0] == "F"]
...
courts_ = [entree for entree in prenoms if len(entree[1]) <= 3]
...
longs_ = [entree for entree in prenoms if len(entree[1]) > 10]
assert longs == longs_, "Erreur sur longs"

# nicolas_2018 est la liste de toutes les entrées correspondant
# au prénom "Nicolas"
# pour l'année 2018
nicolas_2018_ = [entree for entree in prenoms if entree[1] == "NICOLAS" and entree[2] == 2018]
...
entre_5_et_10_mille_ = [entree for entree in prenoms if 5_000 <= entree[3] <= 10_000]
...
fille_10_000_ = [entree for entree in prenoms if entree[0] == "F" and entree[3] >= 10_000]
...
composes_ = [entree for entree in prenoms if "-" in entree[1]]
...
a_debut_ = [entree for entree in prenoms if entree[1][0] == "A"]
